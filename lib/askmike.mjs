...
export async function handleAskMike({ request, model, corpus }) {
  ...
  // ponytail: generateText, not streamText. workers-ai-provider's doStream
  // double-emits every token ("MikeMike built..."); doGenerate is clean. The
  // whole answer returns in ~3-4s on Workers AI, so we wrap the single result
  // in a one-block UI-message stream — useChat still consumes it, and there's
  // no token-by-token streaming to corrupt. Revisit if the provider fixes doStream.
  const stream = createUIMessageStream({
    execute: async ({ writer }) => {
      const { text } = await generateText({ model, system, messages: modelMessages, maxOutputTokens: 500 });
